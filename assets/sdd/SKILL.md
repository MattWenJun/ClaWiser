---
name: sdd
description: 场景驱动开发（Scenario-Driven Development）。动手之前，先想清楚三层：场景是什么、用户真正需要什么、我在这个场景里是什么角色。触发场景：(1) 启动新项目 (2) 从零设计一个功能 (3) 方向不明确需要先想清楚。当用户说"SDD一下"、"想想场景"、"设身处地想想"时触发。不适用于：已有明确方向只需执行的场景。
version: 0.3.0
author: MindCode
