...
在 merge 脚本里加偏移检测逻辑，每次执行后对比当前统计与基线：

触发条件：
- 压缩率偏离基线超过 15 个百分点
- 出现未识别的噪声 pattern
- 某个 reason 的 skip 数量突然归零

触发动作：
1. merge 脚本输出警告标记
2. agent 在 heartbeat 中看到警告后，自动重跑第 2-4 步
3. 新规则写入 merge 脚本，下次 merge 生效
4. 连续 2 次自动修复后偏移仍未收敛 → 告警用户
